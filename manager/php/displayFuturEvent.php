...
WHERE TIMESTAMP(`event`.`EventDate`) >= TIMESTAMP(NOW()) AND `event`.`EventShopId` = $shopId
...
while ($event = $requete->fetch())
  {
    ...
    <div class="box">
        ...
        <div class="overlay">
          <a href="" style="--i:3;" class="fas fa-user" id="'.$event['EventId'].'"></a>
          <a href="" style="--i:3;" class="fas fa-file-alt" id="'.$event['EventId'].'"></a>
          <a href="" style="--i:3;" class="fas fa-pencil-alt" id="'.$event['EventId'].'"></a>
        </div>
    </div>
    ';
  }else{
